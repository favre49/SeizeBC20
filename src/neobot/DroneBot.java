...
public strictfp class DroneBot extends Globals
{
	...
	public static void run(RobotController rc) throws GameActionException
	{
		...
		if (roundNum > 600)
		{
			...
			else
			{
				...
				System.out.println("I know where the HQ is");
		...
		if (shouldCarryTeammate && roundNum < 600)
		{
			pickUpMiners();
			navigate(baseLoc);
		}
		else if (!shouldCarryTeammate && roundNum < 600)
		{
			if (currentPos.distanceSquaredTo(soupLocation) > 45)
			{
				shouldCarryTeammate = true;
			}
			else
				navigateAroundNetGuns(baseLoc);
		}
	...
	public static void navigateAroundNetGuns(MapLocation dest) throws GameActionException
	{
		...
		else
		{
			...
			{
				if(rc.canMove(currentPos.directionTo(bugDestNG)) && !inNetGunRange(bugDestNG))
				{
					bugTracingNG = false;
					return;
				}
